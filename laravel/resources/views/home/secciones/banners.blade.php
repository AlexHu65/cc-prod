...
<div class="owl-banner owl-carousel owl-theme  z-0">
  ...
  <div class="item d-flex justify-content-center align-items-center">
      <img src="{{asset('storage/') .'/'. $banner->img}}" alt="Christian Cruz">
        <div class="text-center w100 absolute text-light animated bounceInLeft delay-2s">
          @if($banner->title_activo)
          <h1 class="d-none d-md-block">
          <span class="bebas-bold s70 text-light">{{(isset($titulo) ? $titulo: 'CONOCE MI TRABAJO')}} </span>
          </h1>
          @endif

          @if(!isset($titulo) && ($banner->title_activo))
          <div class="desk">
              <span class="bebas-bold bg-header s30">{{$banner->titulo}}</span>
          </div>          
          @endif
        </div>
    </div>
